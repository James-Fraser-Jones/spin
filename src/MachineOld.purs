...
--this is actually the fixed point of (Moore'' i o) !?!?!?
--does this actually have finite memory? yes because the definition must be recursive to be finished which means a finite cyclic loop, but it can be arbitrarily large
--do any of them have? only if type s is finite I guess?
--in a sense, if s = Nat, for example, s is infinite
